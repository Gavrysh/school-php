...
function scrollNote() {
    $('html, body').animate({
        scrollTop: $('#comm-show').offset().top
    }, 2000);
}
...
function addComment()
{
    var x = document;
    clearErrors();
    x.getElementById('inform').innerHTML = '';
    if (checkComment()) {
        $.ajax({
            url: '/admin/comments?ajax=1',
            type: "POST",
            cache: false,
            timeout: 5000,
            data: {
            ...
            success: function(msg) {
                ...
                if (response.status) {
                    for ( var key in response ) {
                        if (key != 'status') {
                            x.getElementById('comm-show').innerHTML = x.getElementById('comm-show').innerHTML + '<div class="inner-wrap"><div class="panel-buttons"><a href="/admin/comments/main/delete/' + response[key].id + '" onclick="return confirm(\'Ви лійсно бажаєте видалити цей запис?\');">Видалити</a>&nbsp;<a href="/admin/comments/edit/' + response[key].id + '">Редагувати</a></div><hr><div class="comments-out-name">' + response[key].name + '</div><div class="comments-out-date">' + response[key].date + '</div><div class="comments-out-text">' + response[key].comment + '</div></div>';
                        }
                    }
                    clearInput();
        ...
        $.ajax({
            ...
            success: function(msg) {
                ...
                if (response.status) {
                    ...
                    for (var key in response) {
                        if (key != 'status') {
                            x.getElementById('chat-text').innerHTML = x.getElementById('chat-text').innerHTML + '<span class="notice-date">'+response[key].date+'</span> <span class="notice-user">'+response[key].user+'</span><br><span class="notice-text">'+response[key].text+'</span><br>';
                        }
                    }
                    clearInput('notice');
                    x.getElementById('show-info').innerHTML = 'Запис був успішно доданий!';
